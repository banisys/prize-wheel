    ...
    <div class="row">
      <div class="col-12">
        <div class="p-3 border mt-4 rounded">
          <p class="fw-bold">پیامک ها</p>

          <label for="number" class="form-label">تعداد</label>
          <input type="number" min="100" max="1000" class="form-control col-6" id="number" v-model="number">
          <h6 class="mt-3"> مبلغ: {{ numberFormat(number * sms_amount) }} تومان</h6>
...
export default {
  ...
  methods: {
    submit() {
      axios.post(`${this.$root.apiUrl}/orders/sms`, {
        number: this.number
      }).then(res => {
